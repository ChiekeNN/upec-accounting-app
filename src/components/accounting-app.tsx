...
import { Activity, Bell, BookOpen, ChevronDown, ChevronRight, CircleHelp, FileBarChart2, Landmark, LayoutDashboard, LogOut, Menu, Plus, Search, Settings2, ShieldCheck, Target, Users, WalletCards, X } from "lucide-react";
import type { Snapshot } from "@/lib/types";
import { canEditRecords, hasAdminAccess, roleLabel } from "@/lib/roles";
...
type Section = "dashboard" | "transactions" | "accounts" | "budgets" | "reports" | "audit" | "director" | "team" | "settings";
...
/** Director Admin — Administrators and Directors only. */
const oversightNav: { key: Section; label: string; icon: typeof Landmark }[] = [
  { key: "director", label: "Director Admin", icon: Landmark },
];
...
const sectionNames: Record<Section, string> = { dashboard: "Overview", transactions: "Transactions", accounts: "Chart of accounts", budgets: "Budget monitoring", reports: "Financial reports", audit: "Audit trail", director: "Director Admin", team: "Team & access", settings: "Settings" };
/** Sections an Administrator or Director may open. */
const restrictedSections: Section[] = ["team", "director"];
...
export default function AccountingApp({ initialData }: { initialData: Snapshot }) {
  ...
  const adminLevel = hasAdminAccess(data.user.role);
  const canEdit = canEditRecords(data.user.role);
  const permitted = useCallback((next: Section) => !(restrictedSections.includes(next) && !adminLevel), [adminLevel]);

  useEffect(() => {
    function syncHash() {
      const hash = window.location.hash.replace(/^#\/?/, "") as Section;
      if (hash in sectionNames && permitted(hash)) setSection(hash);
    }
    syncHash();
  ...
  function openTransaction(id?: string) {
    if (!canEdit) { setToast({ message: "Your role has read-only access.", error: true }); return; }
    setTransactionId(id); setModal("transaction");
  }
  function openAccount(id?: string) {
    if (!canEdit) { setToast({ message: "Your role has read-only access.", error: true }); return; }
    setAccountId(id); setModal("account");
  }
  function openBudget(id?: string) {
    if (!canEdit) { setToast({ message: "Your role has read-only access.", error: true }); return; }
    setBudgetAccountId(id); setModal("budget");
  }
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobileOpen ? "sidebar-open" : ""}`}>
      ...
      <div className="sidebar-nav-scroll"><nav aria-label="Main navigation"><div className="sidebar-group-label">WORKSPACE</div>{primaryNav.map(navItem)}{adminLevel && <><div className="sidebar-group-label governance-label">OVERSIGHT</div>{oversightNav.map(navItem)}</>}<div className="sidebar-group-label governance-label">GOVERNANCE</div>{governanceNav.filter((item) => item.key !== "team" || adminLevel).map(navItem)}</nav></div>
